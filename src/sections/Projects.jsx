...
const useIsMobile = (query = '(max-width: 639px)') => {    // custom hook to detect if the device is mobile based on media query and for mobile we give different structure to images.
  ...
  useEffect(() => {     // this useEffect will run only on client side
    ...
    const mql = window.matchMedia(query);     // create a media query list object and it gives the current state of the media query
...
const Projects = () => {
  ...
  const sceneRef = React.useRef(null);    // ref for the section to track scroll progress
  ...
  const { scrollYProgress } = useScroll({             // to track the scroll progress of the section,  scrollYProgress will give a value between 0 and 1
    target: sceneRef,
  ...
  const [activeIndex, setActiveIndex] = React.useState(0);    // state to track the active project index

  useMotionValueEvent(scrollYProgress, "change", (v) => {     // listen for changes in scrollYProgress
  ...
  const activeProject = projects[activeIndex];    // get the active project based on activeIndex
        ...
        <div className={`relative w-full flex-1 flex items-center justify-center ${isMobile ? '-mt-4' : ''}`}>
          {projects.map((project, index) => (
            <div key={project.title}
              ...
              <div className={`relative w-full mt-5 overflow-hidden bg-black/20 shadow-2xl md:shadow-[0_35px_60px_-15px_rgba(0,0,0,0.7)] ${isMobile ? 'mb-6 rounded-lg' : 'mb-10 sm:mb-12 rounded-xl'} h-[62vh] sm:h-[66vh]`}
                style={{
                  zIndex: 10,
